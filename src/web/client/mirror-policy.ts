 ...
 *
 * The discriminator is `cursor === null` (true tour-load —
 * `computeTourOpenSeed` is about to seed from `#<ann-id>`, so we defer the
 * URL write to avoid a strip-then-restore in one cycle), NOT
 * `cursorCardId === null` —
