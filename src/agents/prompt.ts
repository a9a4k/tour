...
// annotation and the rest of the thread chain. The "write your reply
...
export function userPrompt(envelope: ReplyEnvelope): string {
  return `A human reviewer just left a note in Tour ${envelope.tour.id}. The JSON envelope below contains the tour metadata, the triggering annotation, and the full thread chain.

<envelope>
${JSON.stringify(envelope, null, 2)}
</envelope>

Read the triggering_annotation and the thread, then write your reply directly to stdout. Reply to the triggering annotation (id: ${envelope.triggering_annotation.id}).`;
}
