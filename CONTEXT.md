...
- **CLI surface**: `tour create --head <ref> [--base <ref>] [--title <s>]` resolves both ends to SHAs at create time. `--head WORKTREE` snapshots uncommitted changes. `--base` default is context-sensitive: `<head>^` for commit tours, `HEAD` for `WORKTREE` tours. Verbs: `create`, `annotate`, `list`, `show`, `close`, `delete`, `prune`, `tui`, `serve`. Bare `tour` opens TUI on the most recent open Tour.
- **On-disk schema**: `tour.toml` holds `id`, `title?`, `status`, `created_at`, `closed_at?`, `head_sha`, `base_sha`, `head_source`, `base_source`, `worktree_snapshot`. `annotations.jsonl` is one annotation per line: `{id, file, side, line_start, line_end, body, author, created_at}`. `author` is a free-form string (e.g. `"agent"`, `"claude-code"`).
