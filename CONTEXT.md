...
- **Tour picker (v1)**: the webapp opens a Tour-switcher modal on `t` (top-level binding, focus-agnostic, ignored when an editable element has focus) or by clicking the Tour title in the top bar. The modal lists every Tour from `.tour/` (open + closed) sorted newest-first; each row shows the status glyph (`●` open / `○` closed), relative age (`just now` / `Nm ago` / `Nh ago` / `Nd ago` / `Nw ago` / `Nmo ago` / `Ny ago`), title (`(untitled)` fallback), and an annotation-count badge when non-zero. Tour ID is hidden from the row. The current Tour is highlighted with a coloured background (`#1f6feb22`); the initial cursor lands on the first non-current row, or index 0 if the list is all-current. Keys inside the modal: j / ArrowDown, k / ArrowUp, Enter to commit, click row to commit, `t` and Esc to close, click scrim to close. While the picker is open, `n` / `p` / `l` are inert. The modal opens even when only one Tour exists. Snapshot-lost Tours appear in the list with no special marker; selecting one falls through to the existing in-Tour banner. URL: on user-initiated switch, `history.pushState({ tourId }, "", "/?tour=<id>")`; on mount, `?tour=<id>` takes precedence over `window.__INITIAL_TOUR_ID__`; `popstate` syncs `tourId` back to the query param. State reset on Tour switch: `selectedFile`, `currentAnnotationId`, `collapsedOverrides`, and `collapsedFolders` are cleared; `layout` is preserved (per PRD #36). No search / filter v1 — deferred until tour counts justify it (~30+).
