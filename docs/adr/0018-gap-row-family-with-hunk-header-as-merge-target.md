...
> **Status:** The row vocabulary (`gap-mid-top`; `hunk-header` interactive iff `gapAbove > 0`) and the `Shift+Enter` / shift-click "expand all" escape hatch are superseded by [ADR 0025 (directional hunk expand buttons)](./0025-directional-hunk-expand-buttons.md). `gap-mid-top` is replaced by the three directional subkinds `expand-up` / `expand-down` / `expand-all`; the `hunk-header` row is display-only at every `gapAbove`; the `Shift+Enter` / shift-click shortcut is replaced by a per-file **Expand all hidden** button in the file header. The asymmetric file-top merge into `hunk-header`, the `2N = 40` threshold, the **D1** spatial-position convention, the `N = 20` step size, and the surface-parity principle (same `PlannedRow[]` on both surfaces) all stand.
