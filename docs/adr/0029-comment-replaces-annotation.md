...
### `tour show --json` / `tour pickup --json` envelope keys: intentionally renamed

`tour show --json` and `tour pickup --json` emit envelope objects whose top-level keys include the array of Comments. Stage B renamed those envelope keys from `annotations` to `comments` alongside the rest of the identifier sweep. This is a wire-format change for any downstream agent script that did `data.annotations.map(...)` on `tour show` / `tour pickup` output — recorded explicitly so the change isn't surprising later. The decision is intentional: Tour is pre-1.0; the rename is internally consistent with every other vocabulary surface; and the per-record schema (`id`, `file`, `side`, `line_start`, `line_end`, `body`, `author`, `author_kind`, `created_at`, `replies_to`, `kind`) is unchanged, so consumers parsing fields rather than the envelope shape are unaffected. The CLI verb's permanent `tour annotate` alias already covers script compatibility for write paths; envelope-key compatibility for read paths is not symmetric and is not added.

## Lessons for the next rename

Two scope gaps surfaced during the Stage A + Stage B rollout that future rename briefs should pre-empt:

- **Enumerate every prose surface, not just CONTEXT.md.** Stage A's #336 brief and Stage B's #341 brief both swept their named scopes correctly but neither named `skills/tour/`, `README.md`, or `AGENTS.md` explicitly. The result was a follow-up sweep when stale prose surfaced — including a real wire-format-drift bug in `skills/tour/REFERENCE.md` where the documented `tour pickup --json` shape no longer matched the code. Future rename briefs should explicitly enumerate **every** agent- and human-facing doc surface in the "prose sweep" scope: `CONTEXT.md`, `README.md`, `AGENTS.md`, `CHANGELOG.md`, every `skills/*/SKILL.md` / `EXAMPLES.md` / `REFERENCE.md`, every `docs/` page consumed by users or agents.

- **Distinguish "wire-format protection" for records vs. envelopes.** Stage B's #341 brief said "No JSON wire-format change" and enumerated the per-record keys (`id`, `file`, `side`, …) as protected — but did not mention envelope keys (`{ ..., annotations: [...] }` returned by `tour show --json` / `tour pickup --json`). The agent flipped the envelope key as part of the consistency sweep, which was internally reasonable but slipped under the brief's literal protection clause. Future rename briefs that intend to protect wire format should be explicit about whether envelope keys are also protected, name them by example, and decide whether to add a back-compat alias (this ADR chose not to).
