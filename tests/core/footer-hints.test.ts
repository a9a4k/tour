...
describe("composeFooterHints (core, surface: web)", () => {
  ...
  it("emits the diff-mode webapp legend with `Esc: sidebar` when showSendHint is false", () => {
    expect(composeFooterHints({ surface: "web" })).toBe(
      "j/k: move  ·  h/l: side  ·  n/p: nav  ·  c: comment  ·  r: reply  ·  y: yank  ·  L: layout  ·  T: picker  ·  Esc: sidebar",
    );
  });
  ...
  it("inserts `s: send to {agent}` after `r: reply` when reply-agent is configured", () => {
    ...
    expect(out).toBe(
      "j/k: move  ·  h/l: side  ·  n/p: nav  ·  c: comment  ·  r: reply  ·  s: send to claude  ·  y: yank  ·  L: layout  ·  T: picker  ·  Esc: sidebar",
    );
  });
  ...
  it("never emits any TUI-only key labels", () => {
    ...
    for (const out of cases) {
      ...
      // PRD #356 / issue #358 added webapp `y: yank` parity with the TUI,
      // so `y: yank` is no longer TUI-only.
