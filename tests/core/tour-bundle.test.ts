...
describe("loadTourBundle", () => {
  ...
  describe("ok path", () => {
    it("returns kind:'ok' with tour, annotations, diff, and files", async () => {
      ...
      // No orphan annotations seeded → empty windows
    ...
    it("preserves annotations written to the tour", async () => {
      ...
      expect(bundle.annotations[0].body).toBe("in-hunk note");
  ...
  describe("snapshot-lost path", () => {
    ...
    it("preserves annotations even when snapshot is lost", async () => {
      ...
      if (bundle.kind !== "snapshot-lost") return;
      expect(bundle.annotations).toHaveLength(1);
      expect(bundle.annotations[0].body).toBe("before snapshot loss");
    });
  });
  ...
  describe("orphan-window path", () => {
    it("annotations whose anchor lives in hidden context produce orphanWindows entries", async () => {
