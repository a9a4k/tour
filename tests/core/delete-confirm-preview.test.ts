...
describe("cascadeFor (ADR 0036 Slice D / issue #388)", () => {
  it("parent with surviving replies → { kind: 'parent-with-survivors', survivorCount }", () => {
    ...
    expect(cascadeFor(root, ts)).toEqual({
      kind: "parent-with-survivors",
      survivorCount: 3,
    });
  });

  it("parent with no replies → { kind: 'last-node-in-thread' } (Thread vanishes)", () => {
    const root = mkComment({ id: "p1" });
    const ts = [thread(root, [])];
    expect(cascadeFor(root, ts)).toEqual({ kind: "last-node-in-thread" });
  });

  it("reply leaf with siblings → { kind: 'reply' }", () => {
    const root = mkComment({ id: "p1" });
    const r1 = mkComment({ id: "r1", replies_to: "p1" });
    const r2 = mkComment({ id: "r2", replies_to: "p1" });
    const ts = [thread(root, [r1, r2])];
    expect(cascadeFor(r1, ts)).toEqual({ kind: "reply" });
  });

  it("reply leaf with live parent and no siblings → { kind: 'reply' } (parent + stub stays under live parent)", () => {
    const root = mkComment({ id: "p1" });
    const r1 = mkComment({ id: "r1", replies_to: "p1" });
  ...
  it("reply leaf whose parent is already a [deleted] stub and which is the only reply → 'last-node-in-thread'", () => {
    const root: Comment = {
    ...
    const r1 = mkComment({ id: "r1", replies_to: "p1" });
    const ts = [thread(root, [r1])];
    expect(cascadeFor(r1, ts)).toEqual({ kind: "last-node-in-thread" });
  });

  it("reply target whose parent has no matching Thread entry falls back to { kind: 'reply' }", () => {
