...
// banner are the only cursor-walkable affordances around it. The
// `hunkHeaderCursorStop: false` option suppresses the flat-rows
// promotion that would otherwise turn `hunk-header` rows with
// `gapAbove > 0` into `boundary-top` / `hunk-separator` cursor stops.
describe("flatRows hunkHeaderCursorStop option (PRD #270 / issue #273)", () => {
  it("default: promotes a gapAbove > 0 hunk-header to a cursor stop (legacy / web)", () => {
    const f = fileFromDiff(SIMPLE_DIFF, "x.txt");
    const rows: PlannedRow[] = [
      { kind: "hunk-header", header: "@@ -10,3 +10,3 @@", hunkIndex: 3, gapAbove: 12 },
    ];
