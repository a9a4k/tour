...
import { buildArgs } from "../../src/agents/opencode.js";
...
describe("opencode buildArgs", () => {
  it("uses the `run` subcommand", () => {
    const argv = buildArgs(ENVELOPE, "SYSTEM_PROMPT_TEXT");
    expect(argv[0]).toBe("run");
  });

  it("folds the system prompt into the prompt argument (opencode has no --system-prompt flag)", () => {
    const argv = buildArgs(ENVELOPE, "SYSTEM_PROMPT_TEXT");
