...
`;
  ...
  // Issue #250. Under viewport culling the diff scrollbox measures each
  // row's outer wrapper before its children resolve a height. A wrapper with
  // no `minHeight` and no `flexDirection` can collapse to zero rows, in
  // which case every unified diff-row (or interactive row) is culled even
  // though the inner DiffLine carries `minHeight={1}` on its own outer box.
  // The split row wrapper already declares `flexDirection="row" minHeight={1}`;
  // every other diff-row wrapper must match that shape so all four kinds
  // (split, unified, hunk-header interactive, generic interactive) survive
  // the cull pass identically.
  describe("row wrapper height invariant (issue #250)", () => {
    function rowWrapperOf(tree: unknown): AnyElement | undefined {
      // The outermost <box> emitted by the row map: the element whose
      // children include the DiffLine, or whose grandchildren do for the
      // split case (split's wrapper has two 50% column boxes between).
      return flatten(tree).find(
        (el) =>
          el.type === "box" &&
          el.props["width"] === "100%" &&
          flatten(el.props.children).some((c) => c.type === DiffLine),
      );
    }

    it("unified diff-row wrapper has flexDirection=row and minHeight=1", () => {
      const file = parseFile(SIMPLE_DIFF);
      const rows = planRows(file, [], "unified");
      const addIdx = rows.findIndex((r) => r.kind === "diff-row" && r.type === "addition");
      const tree = callDiffRows({ rows: rows.slice(addIdx, addIdx + 1), layout: "unified" });
      const wrapper = rowWrapperOf(tree);
      expect(wrapper).toBeDefined();
      expect(wrapper!.props["flexDirection"]).toBe("row");
      expect(wrapper!.props["minHeight"]).toBe(1);
    });

    it("interactive hunk-header wrapper has flexDirection=row and minHeight=1", () => {
      const rows: PlannedRow[] = [
        {
          kind: "hunk-header",
          header: "@@ -10,3 +10,3 @@",
    ...
    it("generic interactive-row wrapper has flexDirection=row and minHeight=1", () => {
      const rows: PlannedRow[] = [
        { kind: "interactive", subKind: "hunk-separator", boundaryRef: 1, text: "··· 5 hidden ···" },
      ];
      const tree = callDiffRows({ rows, layout: "split" });
      const wrapper = rowWrapperOf(tree);
      expect(wrapper).toBeDefined();
    ...
    it("split diff-row wrapper retains flexDirection=row and minHeight=1 (regression sentinel)", () => {
      const file = parseFile(SIMPLE_DIFF);
      const rows = planRows(file, [], "split");
      const ctxIdx = rows.findIndex((r) => r.kind === "diff-row" && r.type === "context");
      const tree = callDiffRows({ rows: rows.slice(ctxIdx, ctxIdx + 1), layout: "split" });
      const wrapper = flatten(tree).find(
        (el) =>
          el.type === "box" &&
          el.props["flexDirection"] === "row" &&
          el.props["width"] === "100%" &&
          el.props["minHeight"] === 1,
      );
      expect(wrapper).toBeDefined();
    });
  });
